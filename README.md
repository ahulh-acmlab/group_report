...
| 实验结果 | 通过多任务训练，HyperLearner能够利用给定特征的信息，在不增加推理输入的情况下提高检测性能。在多个行人数据集上的实验结果验证了所提出的HyperLearner算法的有效性。我们的定量实验显示语义信道特征可以帮助检测器辨别困难的正样本和低分辨率的负样本。而表征信道特征可抑制背景的误报并提高高分辨率下的定位精度。 |
